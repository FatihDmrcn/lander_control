...
if __name__ == '__main__':
    ...
    ax1.plot(run.t_axis, y_log[:, 4], label='Beta')
    ax1.set_xlim(run.t_begin, run.t_end)
    ax1.grid(True)
    ax1.legend()

    ax2.plot(run.t_axis, y_log[:, 8], label='Vel Z')
    ax2.plot(run.t_axis, y_log[:, 10], label='Vel Beta')
    ax2.grid(True)
    ax2.legend()

    ax3.plot(run.t_axis, f_log[:, 2], label='Force Z')
    ax3.plot(run.t_axis, f_log[:, 0], label='Force X')
    ax3.grid(True)
    ax3.legend()
    plt.show()
